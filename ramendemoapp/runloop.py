...
#
files = os.listdir("/mnt/test") 	# Get list of files in the target directory
os.chdir("/mnt/test")			# Change current directory to access time stamps for file
files.sort(key=os.path.getmtime)	# Sort file according to last modification time
lastprevious = files[len(files) - 1]	# Last file in array has the kast time stamp as it is the last modified
#
# Leave this here for troubleshooting and debugging
#
print ("Current="+myuuid+" Previous run last file="+lastprevious)
#
# Insert one more row for current fsid tracking
#
#
# If array is zero in size this is the first deployment for the application
#
if newramendemofsid == 0:
   isprimarycluster = 1
   issecondarycluster = 0
...
#
   if last1 == 1:
      ...
      sqlcmd = ("INSERT INTO ramendemo.ramenfsid (id, submit_time, cluster1, cluster2, fsid, prevdate, prevtime, gap) VALUES (?, NOW(), 1, 0, ?, ?, ?, ?);")
#
# Calculate the time difference between now and the time stamp of the last file written to by previous deployment.
# We get this file eventually because of replication over the Ramen setup
#
   workstring=lastprevious[-15:-7]+" "+lastprevious[-6:]
   print ("Workstring to recompose date and time pickup = "+workstring)
...
#
# This can be placed back in for troubleshotting or to view in the pod log (via the oc logs command)
#
   #print (outputline)

#
# Commit whatever we have left in the pipe
#
   conn.commit()
